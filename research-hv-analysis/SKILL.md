...
description: |
  ...
  不要用于简单的名词解释（用户只是问"XX是什么"）、不要用于公众号写作（那个用writing-khazix-style）、不要用于纯标题摘要生成（用wechat-title）。
---
...
以下风格元素直接应用到报告写作中（详细定义请参考 writing-khazix-style skill）：
