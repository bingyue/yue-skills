...
test -f .yue-skills/article-image-illustrator/EXTEND.md && echo "project"
test -f "${XDG_CONFIG_HOME:-$HOME/.config}/baoyu-skills/article-image-illustrator/EXTEND.md" && echo "xdg"
test -f "$HOME/.yue-skills/article-image-illustrator/EXTEND.md" && echo "user"
```

```powershell
# PowerShell (Windows)
if (Test-Path .yue-skills/article-image-illustrator/EXTEND.md) { "project" }
$xdg = if ($env:XDG_CONFIG_HOME) { $env:XDG_CONFIG_HOME } else { "$HOME/.config" }
if (Test-Path "$xdg/baoyu-skills/article-image-illustrator/EXTEND.md") { "xdg" }
if (Test-Path "$HOME/.yue-skills/article-image-illustrator/EXTEND.md") { "user" }
```
...
- Or choose type manually: visual-infographic-generator / scene / flowchart / comparison / framework / timeline / mixed

**Default**: if Step 2 found no strong content signal, the recommended preset MUST be `hand-drawn-edu` (visual-infographic-generator + sketch-notes + macaron — warm cream paper, black hand-drawn lines, soft pastel blocks). This is the universal fallback.
...
|------------|---------|----------|
| `hand-drawn` | sketch-notes | **Default.** Warm cream paper, black hand-drawn lines, pastel blocks — educational visual-infographic-generators, concept explainers, onboarding, general knowledge articles |
...
**Default recommendation**: when Step 2 surfaces no strong content signal, recommend **`hand-drawn-edu`** preset (→ visual-infographic-generator + sketch-notes + macaron) as the primary option in Q1. When the user picks a type manually without a preferred_style, recommend `sketch-notes` first in Q3.
...
```yaml
---
type: visual-infographic-generator
density: balanced
style: blueprint
image_count: 4
...
**Filename**: 01-visual-infographic-generator-concept-name.png
...
```
Prompt Files:
- prompts/01-visual-infographic-generator-overview.md ✓
- prompts/02-visual-infographic-generator-distillation.md ✓
...
```
...
|------------------------|--------|
| Yes (e.g., image-generate-multiapi with Google) | Pass reference images via `--ref` |
