...
function parseArgs(): { dir: string; output?: string } {
  ...
  if (!dir) {
    console.error("Usage: bun merge-to-pdf.ts <content-comic-generator-dir> [--output filename.pdf]");
    process.exit(1);
  }
...
function findComicPages(dir: string): PageInfo[] {
  ...
  if (pages.length === 0) {
    console.error(`No content-comic-generator pages found in: ${dir}`);
...
async function createPdf(pages: PageInfo[], outputPath: string) {
  ...
  pdfDoc.setAuthor("content-comic-generator");
...
async function main() {
  ...
  const dirName = basename(dir) === "content-comic-generator" ? basename(join(dir, "..")) : basename(dir);
