---
name: content-comic-generator
description: Knowledge content-comic-generator creator supporting multiple art styles and tones. Creates original educational content-comic-generators with detailed panel layouts and batch-capable image generation. Use when user asks to create "知识漫画", "教育漫画", "biography content-comic-generator", "tutorial content-comic-generator", or "Logicomix-style content-comic-generator".
version: 1.57.0
metadata:
  openclaw:
    requires:
...
Create original knowledge content-comic-generators with flexible art style × tone combinations.
...
- File path(s) → copy to `refs/NN-ref-{slug}.{ext}` alongside the content-comic-generator output
...
| Script | Purpose |
|--------|---------|
| `scripts/merge-to-pdf.ts` | Merge content-comic-generator pages into PDF |

## File Structure

Output directory: `content-comic-generator/{topic-slug}/`
...
**Pick a backend once per session** using the `## Image Generation Tools` rule at the top. If the backend is a repo skill (e.g., `image-generate-multiapi`), read its `SKILL.md` and use its documented interface rather than its scripts.

**7.1 Character sheet** — generate it (to `characters/characters.png`, aspect `4:3`) when the content-comic-generator is multi-page with recurring characters. Skip for simple presets (e.g., four-panel minimalist) or single-page content-comic-generators. Compress to JPEG before use-as-`--ref` (`sips -s format jpeg -s formatOptions 80 …` on macOS, `pngquant --quality=65-80 …` elsewhere) to avoid payload failures. The prompt file at `characters/characters.md` must exist before invoking the backend.
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.yue-skills/content-comic-generator/EXTEND.md` | Project |
| 2 | `$HOME/.yue-skills/content-comic-generator/EXTEND.md` | User home |
...
- **Step 7.1 character sheet** - recommended for multi-page content-comic-generators, optional for simple presets
...
EXTEND.md lives at `.yue-skills/content-comic-generator/EXTEND.md` (project) or `~/.yue-skills/content-comic-generator/EXTEND.md` (user). Three ways to change it:

- **Edit directly** — open EXTEND.md and change fields. Full schema: `references/config/preferences-schema.md`.
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure content-comic-generator preferences" / "重新配置"). The next run re-triggers first-time setup.
