...
Full workflow for generating knowledge content-comic-generators.
...
test -f .yue-skills/content-comic-generator/EXTEND.md && echo "project"
test -f "${XDG_CONFIG_HOME:-$HOME/.config}/baoyu-skills/content-comic-generator/EXTEND.md" && echo "xdg"
test -f "$HOME/.yue-skills/content-comic-generator/EXTEND.md" && echo "user"
```

```powershell
# PowerShell (Windows)
if (Test-Path .yue-skills/content-comic-generator/EXTEND.md) { "project" }
$xdg = if ($env:XDG_CONFIG_HOME) { $env:XDG_CONFIG_HOME } else { "$HOME/.config" }
if (Test-Path "$xdg/baoyu-skills/content-comic-generator/EXTEND.md") { "xdg" }
if (Test-Path "$HOME/.yue-skills/content-comic-generator/EXTEND.md") { "user" }
```

| Path | Location |
|------|----------|
| `.yue-skills/content-comic-generator/EXTEND.md` | Project directory |
| `$HOME/.yue-skills/content-comic-generator/EXTEND.md` | User home |
...
```bash
test -d "content-comic-generator/{topic-slug}" && echo "exists"
```
...
```
header: "Style"
question: "Which visual style for this content-comic-generator?"
options:
...
```
header: "Focus"
question: "What should the content-comic-generator emphasize? (Select all that apply)"
options:
...
options:
  - label: "Yes, proceed (Recommended)"
    description: "Generate character sheet and content-comic-generator pages"
...
be legible but not distracting from the content-comic-generator panels and storytelling.
...
options:
  - label: "Yes, proceed (Recommended)"
    description: "Generate all content-comic-generator page images"
...
Character sheet is recommended for multi-page content-comic-generators with recurring characters, but **NOT required** for all presets.

**When to generate**:

| Condition | Action |
|-----------|--------|
| Multi-page content-comic-generator with detailed/recurring characters | Generate character sheet (recommended) |
| Preset with simplified characters (e.g., four-panel minimalist) | Skip — prompt descriptions are sufficient |
| Single-page content-comic-generator | Skip unless characters are complex |
...
**Strategy A: Using `--ref` parameter** (e.g., image-generate-multiapi)
...
1. Generate unique session ID: `content-comic-generator-{topic-slug}-{timestamp}`
...
```bash
${BUN_X} {baseDir}/scripts/merge-to-pdf.ts <content-comic-generator-dir>
```
