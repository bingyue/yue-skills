...
**写报告时必须明文标注**：
- ✅ 完整诊断：三层都跑了 → 报告标题正常
- ⚠️ 部分诊断：跑了 1-2 层 → 报告标题加「(部分 — 缺 Layer X，原因：Y)」
...
```
python3 ~/.claude/skills/social-account-doctor/scripts/analyze_image.py <图片路径>
→ 输出 JSON：
...
### 小红书（tikhub xiaohongshu ） — 接口选择见 SKILL.md §9.1（实测 2026-04-22）
| 任务 | 工具 | 关键参数 |
|---|---|---|
| 账号信息 | `xiaohongshu_app_get_user_info` (App V1, **首选**) | `user_id` |
| 用户笔记列表 | `xiaohongshu_app_get_user_notes` (App V1) | `user_id` + `cursor` |
