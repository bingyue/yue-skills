...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.yue-skills/web-markdown-extract/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/yue-skills/web-markdown-extract/EXTEND.md` | XDG |
| 3 | `$HOME/.yue-skills/web-markdown-extract/EXTEND.md` | User home |
...
- **Q2 — Output** (header "Output"): "Default output directory?"
  - "web-markdown-extract (Recommended)" — Save to `./web-markdown-extract/{domain}/{slug}.md`
  - User may pick "Other" and type a custom path
- **Q3 — Save** (header "Save"): "Where to save preferences?"
  - "User (Recommended)" — `~/.yue-skills/` (all projects)
  - "Project" — `.yue-skills/` (this project only)
...
| `default_output_dir` | empty | path or empty | Default output directory (empty = `./web-markdown-extract/`) |
...
| `--chrome-profile-dir <path>` | Chrome user data directory (default: `BAOYU_CHROME_PROFILE_DIR` env or `./yue-skills/chrome-profile`) |
...
**Algorithm**:
1. Determine base directory from EXTEND.md `default_output_dir` or default `./web-markdown-extract/`
