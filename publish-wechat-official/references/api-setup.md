...
2. `<cwd>/.yue-skills/.env` with `WECHAT_APP_ID=...`
3. `$HOME/.yue-skills/.env` with `WECHAT_APP_ID=...`
...
Where to save?
A) Project-level: .yue-skills/.env (this project only)
B) User-level: ~/.yue-skills/.env (all projects)
```
