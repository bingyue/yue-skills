...
| Path | Scope |
|------|-------|
| `.yue-skills/publish-wechat-official/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/yue-skills/publish-wechat-official/EXTEND.md` | XDG |
| `$HOME/.yue-skills/publish-wechat-official/EXTEND.md` | User home |
...
| Profile dir | Shared profile at `yue-skills/chrome-profile` |
...
| API credentials | Follow guided setup in Step 2, or set in `.yue-skills/.env` |
...
1. Generate slug (first 2-4 meaningful words, kebab-case; text-translate-workflow Chinese to English for the slug).
2. Save to `publish-wechat-official/YYYY-MM-DD/<slug>.md` (create directory if needed).
...
**API selected + missing credentials** → run guided setup per `references/api-setup.md` (writes to `.yue-skills/.env`).
...
Files created:
[• publish-wechat-official/YYYY-MM-DD/slug.md (if plain text input)]
[• slug.html (converted)]
```
