...
export function loadWechatExtendConfig(): WechatExtendConfig {
  const paths = [
    path.join(process.cwd(), ".yue-skills", "publish-wechat-official", "EXTEND.md"),
    path.join(
      process.env.XDG_CONFIG_HOME || path.join(homeDir(), ".config"),
      "baoyu-skills", "publish-wechat-official", "EXTEND.md"
    ),
    path.join(homeDir(), ".yue-skills", "publish-wechat-official", "EXTEND.md"),
  ];
...
function resolveCredentialSource(
  ...
  throw new Error(
    ...
    "Set via EXTEND.md account config, environment variables, or .yue-skills/.env file." +
    partialHint
  );
}

export function loadCredentials(account?: ResolvedAccount): LoadedCredentials {
  const cwdEnvPath = path.join(process.cwd(), ".yue-skills", ".env");
  const homeEnvPath = path.join(homeDir(), ".yue-skills", ".env");
  ...
  if (prefix) {
    ...
    sources.push(
      ...
      buildCredentialSource(`<cwd>/.yue-skills/.env (${prefixedKeyLabel})`, cwdEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
      buildCredentialSource(`~/.yue-skills/.env (${prefixedKeyLabel})`, homeEnv, `${prefix}APP_ID`, `${prefix}APP_SECRET`),
    );
  }

  sources.push(
    buildCredentialSource("process.env", process.env, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("<cwd>/.yue-skills/.env", cwdEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
    buildCredentialSource("~/.yue-skills/.env", homeEnv, "WECHAT_APP_ID", "WECHAT_APP_SECRET"),
  );
