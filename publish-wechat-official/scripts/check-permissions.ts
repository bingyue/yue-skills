...
async function checkApiCredentials(): Promise<void> {
  ...
  const projectEnv = path.join(cwd, '.yue-skills', '.env');
  const userEnv = path.join(os.homedir(), '.yue-skills', '.env');
...
async function main(): Promise<void> {
  console.log('=== publish-wechat-official: Permission & Environment Check ===\n');
