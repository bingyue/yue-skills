...
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/daily"
```

后面"工作流"章节的 curl 例子为了简洁默认你已经设了 `$UA`——实际调用必须加 `-H "User-Agent: $UA"`，**不要忘**。漏掉这一步会让你以为接口挂了，实际只是被 403 挡了。

> **范围澄清**：这条 UA 要求**只针对 `/api/public/*` API 端点**。`/news-aihot-daily-skill/{install.sh,SKILL.md,README.md}` 安装入口 nginx 上**特意豁免** UA 黑名单（设计前提就是给 `curl -fsSL ... | bash` 一行装用），用 default curl UA 直通 200。不要把"先决条件"误推广到所有 news-aihot-daily.virxact.com 路径。
...
约定：
- Base URL: `https://news-aihot-daily.virxact.com`
- 鉴权：无（匿名）
...
- 完整 OpenAPI 3.1 规范：`https://news-aihot-daily.virxact.com/openapi.yaml`
...
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=selected&since=$since&take=50"

# 拉最近 50 条精选（用户问"看下精选" / 不带明确时间窗）
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=selected&take=50" \
...
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/daily/2026-05-07"
```
...
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/dailies?take=14" \
...
since=$(date -u -v-24H +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -d '24 hours ago' +%Y-%m-%dT%H:%M:%SZ)
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=all&since=$since&take=100"
```
...
**用户问"公众号最近发什么":items API 不含公众号(mp_hot 信源单独走前端 `/mp` 页),Skill 暂时无法回答这类问题,可以提示用户去 `https://news-aihot-daily.virxact.com/mp` 看公众号爆文页**。

```bash
# 例：拉最近 50 条 AI 论文（默认精选 + paper 类别）
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=selected&category=paper&take=50" \
  | jq '.items[] | {title, source, publishedAt, url}'

# 例：精选里的模型发布
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=selected&category=ai-models&take=20"

# 例外：用户明确说"全部论文 / 所有模型发布"才走 mode=all
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=all&category=paper&take=100"
```
...
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=selected&category=ai-models&since=$since&take=100"

# 拉最近 3 天的精选动态(用户明确说"最近 3 天")
since=$(date -u -v-3d +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -d '3 days ago' +%Y-%m-%dT%H:%M:%SZ)
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=selected&since=$since&take=100"
```
...
```bash
# 第 1 页
resp1=$(curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=all&take=100")
echo "$resp1" | jq '.items | length'   # 100

# 第 2 页
cursor=$(echo "$resp1" | jq -r '.nextCursor')
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=all&take=100&cursor=$cursor"
```
...
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?q=OpenAI&take=30"

# 找 Sora 相关的所有 AI 动态(任何包含 Sora 的标题或摘要)
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?q=Sora"

# 找 RAG 论文(category 限定 + 关键词)
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?category=paper&q=RAG&take=30"

# 关键词 + 时间窗(Anthropic 最近 3 天的精选)
SINCE=$(date -u -v-3d +%Y-%m-%dT%H:%M:%SZ 2>/dev/null || date -u -d '3 days ago' +%Y-%m-%dT%H:%M:%SZ)
curl -sH "User-Agent: $UA" "https://news-aihot-daily.virxact.com/api/public/items?mode=selected&q=Anthropic&since=$SINCE"
```
...
数据源最多写一句：**"数据来自 news-aihot-daily.virxact.com"**，要么干脆不提（用户在用 skill 时已经知道源头）。
