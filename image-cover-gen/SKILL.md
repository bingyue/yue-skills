...
- `independent` (default): `article-cover-generator/{topic-slug}/`
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.yue-skills/article-cover-generator/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/yue-skills/article-cover-generator/EXTEND.md` | XDG |
| 3 | `$HOME/.yue-skills/article-cover-generator/EXTEND.md` | User home |
...
5. **Generate**: Call the chosen backend with the prompt file, output path, aspect ratio.
   ...
   - **Codex `imagegen` (native)** or other runtime-native tools / `image-generate-multiapi` skill: per the rule in `## Image Generation Tools` above.
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure article-cover-generator preferences" / "重新配置"). The next run re-triggers first-time setup.
