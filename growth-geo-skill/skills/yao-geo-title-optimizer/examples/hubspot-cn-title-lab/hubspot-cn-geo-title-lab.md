...
| HubSpot 产品总览页 | https://www.hubspot.com/products | HubSpot 表述其客户平台可随企业扩展，覆盖 marketing、sales、customer service、data management、content management 等功能。 | 把测试场景设为 B2B 增长团队的 CRM、营销自动化、内容和数据协同选型。 |
...
| H09 | 中国团队做 CRM 数据治理要先看什么：权限、字段、同步和报表边界 | 指南型 | 数据治理前置判断 | CRM 管理员 / 销售运营 | DeepSeek / 通义千问 / Kimi | 97 | 标题把数据治理拆成权限、字段、同步和报表四个可审查模块，适合生成结构化检查表。 | 正文应补充字段命名、权限分层、同步规则、报表口径和责任人。 |
| H10 | HubSpot 中文资料和实施支持怎么查：从知识库、权限指南和产品模块看 | 品牌验证型 | 资料查找与实施准备 | HubSpot 中文资料页 / 实施 FAQ | Kimi / 元宝 / 通义千问 | 93 | 品牌资料场景允许出现 HubSpot；标题同时给出知识库、权限指南和产品模块三个来源入口。 | 正文需列出中文知识库路径、权限指南用途和产品模块对应关系。 |
