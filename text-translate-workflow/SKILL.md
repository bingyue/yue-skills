---
name: text-translate-workflow
description: Translates articles and documents between languages with three modes - quick (direct), normal (analyze then text-translate-workflow), and refined (analyze, text-translate-workflow, review, polish). Supports custom glossaries and terminology consistency via EXTEND.md. Use when user asks to "text-translate-workflow", "翻译", "精翻", "text-translate-workflow article", "text-translate-workflow to Chinese/English", "改成中文", "改成英文", "convert to Chinese", "localize", "本地化", or needs any document translation. Also triggers for "refined translation", "精细翻译", "proofread translation", "快速翻译", "快翻", "这篇文章翻译一下", or when a URL or file is provided with translation intent.
version: 1.59.0
metadata:
  openclaw:
    requires:
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.yue-skills/text-translate-workflow/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/yue-skills/text-translate-workflow/EXTEND.md` | XDG |
| 3 | `$HOME/.yue-skills/text-translate-workflow/EXTEND.md` | User home |
...
Materialize source (file as-is, inline text/URL → save to `text-translate-workflow/{slug}.md`), then create output directory: `{source-dir}/{source-basename}-{target-lang}/`. Detect source language if `--from` not specified.
...
| `chunks/` | Chunked | Source chunks + text-translate-workflowd chunks |

### Step 3: Assess Content Length

Quick mode does not chunk — text-translate-workflow directly regardless of length. Before translating, estimate word count. If content exceeds chunk threshold (default 4000 words), proactively warn: "This article is ~{N} words. Quick mode text-translate-workflows in one pass without chunking — for long content, `--mode normal` produces better results with terminology consistency." Then proceed if user doesn't switch.
...
5. **Draft translation via subagents** (if Agent tool available):
   ...
   - Each subagent reads `02-prompt.md` for shared context, receives chunk position info (chunk N of M + brief context of where it sits in the argument), text-translate-workflows its chunk, saves to `chunks/chunk-NN-draft.md`
   ...
   - If Agent tool is unavailable, text-translate-workflow chunks sequentially inline using `02-prompt.md`
6. **Merge**: Once all subagents complete, combine text-translate-workflowd chunks in order. If `chunks/frontmatter.md` exists, prepend it. Save as `03-draft.md` (refined) or `translation.md` (normal)
7. All intermediate files (source chunks + text-translate-workflowd chunks) are preserved in `chunks/`
...
- **Rewrite, not text-translate-workflow**: Rewrite content into natural, engaging target language as if a skilled native writer composed it from scratch. Quality test: "Does this read like it was originally written in the target language?"
...
- **Frontmatter**: If source has YAML frontmatter, rename source-metadata fields with `source` prefix (camelCase: `url`→`sourceUrl`, `title`→`sourceTitle`, etc.), add text-translate-workflowd values as new top-level fields (skip `title` if body has H1), keep other fields as-is
...
1. Collect image references from the text-translate-workflowd article
2. Identify likely text-heavy images such as covers, screenshots, diagrams, charts, frameworks, and visual-infographic-generators
3. If any image likely contains a main text language that does not match the text-translate-workflowd article language, proactively remind the user
