...
| 错误 | 原因 | 解决方案 |
|------|------|----------|
| Playwright 渲染失败 | Chromium 未安装 | `.venv/bin/playwright install chromium` |
| 字体渲染异常 | 中文/Emoji 字体缺失 | 安装 `fonts-noto-cjk` 和 `fonts-noto-color-emoji` |
| 发布失败 401 | Cookie 过期 | 重新获取小红书 Cookie 并更新到凭证文件 |
| 卡片内容溢出 | 单卡文字过多超过 max-height | 减少单卡内容或增加分页（使用 auto_fit 模式） |
| 封面图生成空白 | Markdown 格式错误 | 检查 Markdown 标题和分隔符格式 |
