...
description: First-time setup flow for visual-infographic-generator preferences
---
...
When no EXTEND.md is found, guide the user through preference setup before generating any visual-infographic-generator. Saved preferences shift Step-3 recommendations and Step-4 defaults only — they never bypass Step 4 confirmation (see the `## Confirmation Policy` section in SKILL.md).
...
```
header: "Language"
question: "Output language for visual-infographic-generator text?"
options:
...
options:
  - label: "Project"
    description: ".yue-skills/ (this project only)"
  - label: "User"
    description: "~/.yue-skills/ (all projects)"
```
...
| Choice | Path | Scope |
|--------|------|-------|
| Project | `.yue-skills/visual-infographic-generator/EXTEND.md` | Current project |
| User | `~/.yue-skills/visual-infographic-generator/EXTEND.md` | All projects |

XDG path (`${XDG_CONFIG_HOME:-$HOME/.config}/baoyu-skills/visual-infographic-generator/EXTEND.md`) is also recognized at read time but not offered as a save target during first-time setup.
...
`preferred_image_backend: auto` is the baked-in default — first-time setup never asks about it. The `## Image Generation Tools` rule in SKILL.md then picks the runtime-native tool (Codex `imagegen`, Hermes `image_generate`, etc.) when one is available, and falls back to installed backends like `image-generate-multiapi`.
