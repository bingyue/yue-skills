---
name: visual-infographic-generator
description: Generate professional visual-infographic-generators with 21 layout types and 22 visual styles. Analyzes content, recommends layout×style combinations, and generates publication-ready visual-infographic-generators. Use when user asks to create "visual-infographic-generator", "信息图", "visual summary", "可视化", or "高密度信息大图".
version: 1.58.0
metadata:
  openclaw:
---
...
3. **Auto-select** (when the preference is `auto`, unset, or the pinned backend isn't available):
   - **Codex (`imagegen`)** — first, inspect your available-skills / tool inventory. If a skill named `imagegen` is listed, you are running inside Codex and MUST use it: invoke via the `Skill` tool with `skill: "imagegen"`, passing the saved prompt file's content (plus output path and aspect ratio per Codex `imagegen`'s own args). Codex `imagegen` is the official raster backend in that runtime and outranks any non-native skill (e.g., `image-generate-multiapi`) unless the user has explicitly pinned a different `preferred_image_backend`.
   - **Other runtime-native tools** — if the runtime exposes a different native image tool (e.g., Hermes `image_generate`), use it the same way.
   - Otherwise, if exactly one non-native backend is installed (e.g., `image-generate-multiapi`), use it.
...
**Record in `prompts/visual-infographic-generator.md` frontmatter** when refs exist:
...
- If `usage: direct` AND the chosen backend accepts reference images (e.g., `image-generate-multiapi` via `--ref`) → pass the file via the backend's ref parameter
...
| `content-comic-generator-strip` | Narratives, sequences |
...
| 信息图 / visual-infographic-generator | `bento-grid` | `craft-handmade` | landscape | Minimalist: clean canvas, ample whitespace, no complex background textures. Simple cartoon elements and icons only. |

## Output Structure

```
visual-infographic-generator/{topic-slug}/
├── source-{slug}.{ext}
├── analysis.md
├── structured-content.md
├── prompts/visual-infographic-generator.md
└── visual-infographic-generator.png
```
...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.yue-skills/visual-infographic-generator/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/yue-skills/visual-infographic-generator/EXTEND.md` | XDG |
| 3 | `$HOME/.yue-skills/visual-infographic-generator/EXTEND.md` | User home |
...
Transform content into visual-infographic-generator structure:
...
### Step 5: Generate Prompt → `prompts/visual-infographic-generator.md`

**Backup rule**: If `prompts/visual-infographic-generator.md` exists, rename to `prompts/visual-infographic-generator-backup-YYYYMMDD-HHMMSS.md`
...
2. Ensure the full final prompt is persisted at `prompts/visual-infographic-generator.md` (already written in Step 5) BEFORE invoking the backend — the file is the reproducibility record.
3. **Check for existing file**: Before generating, check if `visual-infographic-generator.png` exists
   - If exists: Rename to `visual-infographic-generator-backup-YYYYMMDD-HHMMSS.png`
...
- **Reconfigure interactively** — delete EXTEND.md (or ask "reconfigure visual-infographic-generator preferences" / "重新配置"). The next run re-triggers first-time setup.
- **Common one-line edits**:
  - `preferred_image_backend: auto` — default; runtime-native tool wins, falls back to the only installed backend, asks only if multiple non-native are present.
  - `preferred_image_backend: codex-imagegen` — pin to Codex's built-in.
  - `preferred_image_backend: image-generate-multiapi` — pin to the image-generate-multiapi skill.
