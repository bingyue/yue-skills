...
description: First-time setup and default model selection flow for image-generate-multiapi
---
...
options:
  - label: "2k (Recommended)"
    description: "2048px - covers, illustrations, visual-infographic-generators"
  - label: "normal"
...
| Choice | Path | Scope |
|--------|------|-------|
| Project | `.yue-skills/image-generate-multiapi/EXTEND.md` | Current project |
| User | `$HOME/.yue-skills/image-generate-multiapi/EXTEND.md` | All projects |
...
- In `image-generate-multiapi`, Azure `--model` / `default_model.azure` should be the Azure deployment name, not just the underlying model family.
...
- In `image-generate-multiapi`, `quality` is a compatibility preset. It is not a native DashScope parameter.
...
- In `image-generate-multiapi`, Z.AI currently exposes text-to-image only; reference images are not wired for this provider.
...
options:
  - label: "google/nano-banana-2 (Recommended)"
    description: "Current default for general Replicate image generation in image-generate-multiapi"
