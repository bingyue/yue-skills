...
test("parseArgs parses the main image-generate-multiapi CLI flags", () => {
...
test("loadExtendConfig renames legacy EXTEND.md when the new path is missing", async () => {
  const root = await makeTempDir("image-generate-multiapi-extend-");
  const cwd = path.join(root, "project");
  const home = path.join(root, "home");
  const legacyPath = path.join(cwd, ".yue-skills", "image-generate-legacy", "EXTEND.md");
  const currentPath = path.join(cwd, ".yue-skills", "image-generate-multiapi", "EXTEND.md");
...
test("loadExtendConfig leaves legacy EXTEND.md untouched when both paths exist", async () => {
  const root = await makeTempDir("image-generate-multiapi-extend-dual-");
  const cwd = path.join(root, "project");
  const home = path.join(root, "home");
  const legacyPath = path.join(cwd, ".yue-skills", "image-generate-legacy", "EXTEND.md");
  const currentPath = path.join(cwd, ".yue-skills", "image-generate-multiapi", "EXTEND.md");
...
test("loadBatchTasks and createTaskArgs resolve batch-relative paths", async (t) => {
  const root = await makeTempDir("image-generate-multiapi-batch-");
...
test("path normalization, worker count, and retry classification follow expected rules", () => {
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("Replicate returned 2 outputs, but image-generate-multiapi currently supports saving exactly one image per request."),
    ),
    false,
  );
  assert.equal(
  ...
  assert.equal(
    isRetryableGenerationError(
      new Error("DashScope wan2.7 image models in image-generate-multiapi support exactly one output image per request."),
    ),
    false,
  );
  assert.equal(
