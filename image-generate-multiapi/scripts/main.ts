...
Env file load order: CLI args > EXTEND.md > process.env > <cwd>/.yue-skills/.env > ~/.yue-skills/.env`);
}
...
async function loadEnv(): Promise<void> {
  ...
  const homeEnv = await loadEnvFile(path.join(home, ".yue-skills", ".env"));
  const cwdEnv = await loadEnvFile(path.join(cwd, ".yue-skills", ".env"));
...
function getExtendConfigPathPairs(cwd: string, home: string): ExtendConfigPathPair[] {
  return [
    {
      current: path.join(cwd, ".yue-skills", "image-generate-multiapi", "EXTEND.md"),
      legacy: path.join(cwd, ".yue-skills", "image-generate-legacy", "EXTEND.md"),
    },
    {
      current: path.join(home, ".yue-skills", "image-generate-multiapi", "EXTEND.md"),
      legacy: path.join(home, ".yue-skills", "image-generate-legacy", "EXTEND.md"),
    },
  ];
}
...
export function detectProvider(args: CliArgs): Provider {
  ...
  throw new Error(
    "No API key found. Set GOOGLE_API_KEY, GEMINI_API_KEY, OPENAI_API_KEY, AZURE_OPENAI_API_KEY+AZURE_OPENAI_BASE_URL, OPENROUTER_API_KEY, DASHSCOPE_API_KEY, ZAI_API_KEY, MINIMAX_API_KEY, REPLICATE_API_TOKEN, JIMENG keys, or ARK_API_KEY.\n" +
      "Create ~/.yue-skills/.env or <cwd>/.yue-skills/.env with your keys."
  );
}
