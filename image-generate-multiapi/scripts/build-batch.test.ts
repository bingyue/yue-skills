...
const scriptPath = path.join(repoRoot, "skills", "image-generate-multiapi", "scripts", "build-batch.ts");
...
async function makeFixture(): Promise<{
  ...
  const root = await fs.mkdtemp(path.join(os.tmpdir(), "image-generate-multiapi-build-batch-"));
...
test("build-batch omits default model so image-generate-multiapi can resolve env or EXTEND defaults", async () => {
