...
| Path | Scope |
|------|-------|
| `.yue-skills/image-generate-multiapi/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/yue-skills/image-generate-multiapi/EXTEND.md` | XDG |
| `$HOME/.yue-skills/image-generate-multiapi/EXTEND.md` | User home |
...
Legacy compatibility: if `.yue-skills/image-generate-legacy/EXTEND.md` exists and the new path doesn't, the runtime renames it to `image-generate-multiapi`. If both exist, the runtime leaves them alone and uses the new path.
...
**Load priority**: CLI args > EXTEND.md > env vars > `<cwd>/.yue-skills/.env` > `~/.yue-skills/.env`
...
| `2k` (default) | 2K | 2048px target | 2K | 2K | Covers, illustrations, visual-infographic-generators |
...
| Input is `outline.md` + `prompts/` (e.g. from `article-image-illustrator`) | Batch — use `scripts/build-batch.ts` to assemble the payload | The outline + prompt files already contain everything needed |
