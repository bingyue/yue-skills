...
function parseArgs(): { dir: string; output?: string } {
  ...
  if (!dir) {
    console.error("Usage: bun merge-to-pdf.ts <slides-deck-generator-dir> [--output filename.pdf]");
    process.exit(1);
  }
...
async function createPdf(slides: SlideInfo[], outputPath: string) {
  ...
  pdfDoc.setAuthor("slides-deck-generator");
...
async function main() {
  ...
  const dirName = basename(dir) === "slides-deck-generator" ? basename(join(dir, "..")) : basename(dir);
