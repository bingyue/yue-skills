...
| `editorial-visual-infographic-generator` | clean + cool + editorial + dense | Tech explainers, research |
...
| hand-drawn, visual-infographic-generator, diagram, process, onboarding | `hand-drawn-edu` |
...
| explainer, journalism, science communication | `editorial-visual-infographic-generator` |
...
- File path → copy to `{slides-deck-generator-dir}/refs/NN-ref-{slug}.{ext}`
...
At generation time, verify files exist. If `usage: direct` and the backend accepts refs (e.g., `image-generate-multiapi --ref`), pass the file on every slide. Otherwise embed extracted `style`/`palette` traits in the prompt text.

## File Layout

```
slides-deck-generator/{topic-slug}/
├── source-{slug}.{ext}
├── outline.md
...
| Path | Scope |
|------|-------|
| `.yue-skills/slides-deck-generator/EXTEND.md` | Project |
| `${XDG_CONFIG_HOME:-$HOME/.config}/yue-skills/slides-deck-generator/EXTEND.md` | XDG |
| `$HOME/.yue-skills/slides-deck-generator/EXTEND.md` | User home |
...
**1.3 Check existing output** ⚠️ REQUIRED before Step 2. If `slides-deck-generator/{topic-slug}/` exists, ask how to proceed — four options (regenerate outline / regenerate images / backup and regenerate / exit), verbatim copy in `references/confirmation.md`.
...
```bash
${BUN_X} {baseDir}/scripts/merge-to-pptx.ts <slides-deck-generator-dir>
${BUN_X} {baseDir}/scripts/merge-to-pdf.ts <slides-deck-generator-dir>
```
