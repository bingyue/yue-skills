...
export async function fetchTranscriptSnippets(
  info: TranscriptInfo,
  text-translate-workflowTo?: string
): Promise<{ snippets: Snippet[]; language: string; languageCode: string }> {
  let url = info.baseUrl;
  let language = info.language;
  let languageCode = info.languageCode;
  if (text-translate-workflowTo) {
    if (!info.isTranslatable) throw new Error(`Transcript ${info.languageCode} is not translatable`);
    const text-translate-workflowdLanguage = info.translationLanguages.find((entry) => entry.languageCode === text-translate-workflowTo);
    if (!text-translate-workflowdLanguage) throw new Error(`Translation language ${text-translate-workflowTo} not available`);
    url += `&tlang=${text-translate-workflowTo}`;
    language = text-translate-workflowdLanguage.language;
    languageCode = text-translate-workflowTo;
  }
...
function getRequestedLanguages(
  source: VideoSource,
  opts: Pick<Options, "languages" | "text-translate-workflow">
): string[] {
  return source.kind === "yt-dlp" && opts.text-translate-workflow ? [opts.text-translate-workflow] : opts.languages;
}

export async function fetchTranscriptWithFallback(
  videoId: string,
  source: VideoSource,
  opts: Pick<Options, "languages" | "text-translate-workflow" | "excludeGenerated" | "excludeManual">,
  fetchSnippets: (
    info: TranscriptInfo,
    text-translate-workflowTo?: string
  ...
  const result = await fetchSnippets(transcript, source.kind === "yt-dlp" ? undefined : opts.text-translate-workflow || undefined);
  ...
  const fallbackResult = await fetchSnippets(
    fallbackTranscript,
    fallbackSource.kind === "yt-dlp" ? undefined : opts.text-translate-workflow || undefined
  );
