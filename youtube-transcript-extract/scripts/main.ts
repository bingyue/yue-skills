...
async function processVideo(videoId: string, opts: Options): Promise<VideoResult> {
  ...
  if (!needsFetch && videoDir) {
    ...
    const wantedLanguages = opts.text-translate-workflow ? [opts.text-translate-workflow] : opts.languages;
...
Options:
  ...
  --text-translate-workflow <code>           Translate to language code
  ...
  --output-dir <dir>           Base output directory (default: youtube-transcript-extract)
  -h, --help                   Show help`);
}
...
function parseArgs(argv: string[]): Options | null {
  ...
  for (let i = 0; i < argv.length; i++) {
    ...
    } else if (arg === "--text-translate-workflow") {
      opts.text-translate-workflow = argv[++i] || "";
