...
description: First-time setup and default model selection flow for image-generate-legacy
---
...
| Choice | Path | Scope |
|--------|------|-------|
| Project | `.yue-skills/image-generate-legacy/EXTEND.md` | Current project |
| User | `$HOME/.yue-skills/image-generate-legacy/EXTEND.md` | All projects |
...
- In `image-generate-legacy`, Azure `--model` / `default_model.azure` should be the Azure deployment name, not just the underlying model family.
...
- In `image-generate-legacy`, `quality` is a compatibility preset. It is not a native DashScope parameter.
...
- Z.AI does not accept reference images or `n > 1` in `image-generate-legacy`; use Google/OpenAI providers for those workflows.
