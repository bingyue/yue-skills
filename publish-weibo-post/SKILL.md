...
| Priority | Path | Scope |
|----------|------|-------|
| 1 | `.yue-skills/publish-weibo-post/EXTEND.md` | Project |
| 2 | `${XDG_CONFIG_HOME:-$HOME/.config}/yue-skills/publish-weibo-post/EXTEND.md` | XDG |
| 3 | `$HOME/.yue-skills/publish-weibo-post/EXTEND.md` | User home |
...
If a script fails with `Chrome debug port not ready` or `Unable to connect`, kill only the CDP Chrome instances (those with `--remote-debugging-port` AND the `yue-skills` profile), then retry:

```bash
pkill -f "remote-debugging-port.*yue-skills/chrome-profile" 2>/dev/null; sleep 2
```

**CRITICAL**: Never kill all Chrome processes (`pkill -f "Google Chrome"`). Only kill Chrome instances launched by CDP with the `yue-skills` profile directory. The user may have regular Chrome windows open.
