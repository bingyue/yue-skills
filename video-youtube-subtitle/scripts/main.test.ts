...
test("buildTranscriptListFromYtDlp keeps manual and generated tracks separate", () => {
  ...
  const text-translate-workflowd = findTranscript(transcripts, ["zh-Hans"], false, false);
  assert.equal(text-translate-workflowd.languageCode, "zh-Hans");
  assert.equal(text-translate-workflowd.isGenerated, true);
});
